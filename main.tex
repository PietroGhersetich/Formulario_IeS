...
% Il mio driver di stampa aggiunge, per qualche motivo, dei margini aggiuntivi; sono sufficienti.
\usepackage[a4paper, landscape, left=0.1cm, right=0.1cm, top=0.1cm, bottom=0.1cm]{geometry}
        ...
        \item \textbf{Proc. di Poisson}: $PP(\lambda)$ è la versione continua dei B.P.
              \begin{itemize}
                  \item \textit{Probabilità di avere $k$ arrivi nell'intervallo $[0, \tau]$}:  $N[0, \tau]\sim Poisson(\lambda\tau)$ è una v.a. di Poisson.
                        ...
                        \end{itemize}
                  \item \textit{Tempo di interarrivo}: $T_{i}\sim\mathcal{E}(p)$. Tutti i tempi di interarrivo sono indipendenti e godono di perdita di memoria.
                  \item \textit{Distribuzione del tempo al $k$-esimo arrivo}: $Y_{k}\sim Erlang-k(\lambda)$, ossia una somma di $k$ esponenziali (interarrivi).
                        \begin{itemize}
                  ...
                  \item \textit{Incidenza casuale per P.P.} Dato un P.P. iniziato da un tempo indefinito $t$, il tempo tra l'arrivo precedente a $t$ e quello successivo è una $Erlang-2(\lambda)$.
              \end{itemize}
        \item Relazione Bernoulli/Poisson:
